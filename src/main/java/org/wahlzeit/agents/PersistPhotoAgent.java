...
import org.wahlzeit.model.ShirtPhoto;
import org.wahlzeit.model.ShirtPhotoManager;
...
public class PersistPhotoAgent extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String id = request.getParameter(ShirtPhoto.ID);
		log.config(LogBuilder.createSystemMessage().addParameter("Try to persist PhotoId", id).toString());
		if (id != null && !"".equals(id)) {
			ShirtPhoto photo = (ShirtPhoto) ShirtPhotoManager.getInstance().getPhoto(id);
			if (photo != null) {
				ShirtPhotoManager.getInstance().savePhoto(photo);
